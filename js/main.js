...
$__System.register('b', ['9', 'a'], function (_export) {
	...
	function track_click_row($this) {
		...
		var ignore = 0;
		var isAccordion = 0;
		if ($this.parent().parent().hasClass('nol1') && $this.data('child')) {
			isAccordion = 1;
		}
		//lg(`isAccordion: ${isAccordion}`);
		...
		if (!ignore) {
			if (isAccordion) {
				if (!$this.hasClass('active')) {
					log_action('EXPAND: ' + this_row);
				} else {
					log_action('COLLAPSE: ' + this_row);
				}
			} else {
...
$__System.register('c', ['7', '9', 'a', 'b'], function (_export) {
	...
	var _, $, lg, track_start_session, track_click_row, track_click_back, track_toggle_menu, track_click_overlay, track_click_search, track_get_search_string, $menuButton, $menuDrawer, $menuDrawerX, $slider, $menuL1Link, $menuNoL1Link, $backBar, $menuBack, $groups, $groupNoL1, $groupNoL1Rows, $header, $row, $rowStoreAndRewards, $rowGroup, $overlay, $searchInput, $searchButton, menuState, headerHeight, id, child, level, urlParams, variant;
	...
	function hide_menu() {
		...
		$groupNoL1.css({ 'height': 0 });
		$groupNoL1Rows.removeClass('active');
	}
	// ----------------- MENU SHIFT FORWARD -----------------
	...
	// ----------------- TOGGLE OPEN -----------------
	function toggle_open($this) {
		// Get all already inserted groups
		var $insertedChildGroups = $this.parent().find('.group');
		// Vars to check inserted groups
		var child_exists = 0;
		var $childExistingGroup = undefined;
		$insertedChildGroups.map(function (v, k) {
			var group = $(k).data('group');
			if (group === child) {
				child_exists = 1;
				$childExistingGroup = $(k);
			}
		});
		// If already inserted, open the group. Else, need to insert it first.
		if (child_exists) {
			// OPEN THE CHILD GROUP
			accordion_expand($childExistingGroup);
		} else {
			// INSERT THE CHILD GROUP
			// THEN OPEN THE CHILD GROUP
			var group_exists = 0;
			var $groupMatch = undefined;
			$groups.map(function (v, k) {
				var group = $(k).data('group');
				if (group === child) {
					group_exists = 1;
					$groupMatch = $(k);
			...
			if (group_exists) {
				$groupMatch.insertAfter($this);
				$groupMatch.addClass('active');
				accordion_expand($groupMatch);
			}
		}
	}
	// ----------------- TOGGLE CLOSE -----------------
	function toggle_close($this) {
		var $insertedChildGroups = $this.parent().find('.group');
		var $childExistingGroup = undefined;
		$insertedChildGroups.map(function (v, k) {
			var group = $(k).data('group');
		...
		accordion_close($childExistingGroup);
	}
	// ----------------- EXPAND ACCORDION GROUP -----------------
	function accordion_expand($this) {
		var target_height = $this.find('.rows').outerHeight();
		$this.css({ 'height': target_height });

		var $closestParent = $this.parent().closest('.group:not(.l2, .l1)');
		var x = $closestParent.length;
		var asdfasdf = $closestParent.data('group');
		if (x) {
			$closestParent.css({ 'height': 'auto' });
		}
	}
	// ----------------- CLOSE ACCORDION GROUP -----------------
	function accordion_close($this) {
		$this.css({ 'height': $this.outerHeight() });
		var timer = setTimeout(function () {
			$this.css({ 'height': 0 });
		}, 10);
		// Close all children
		var $childGroups = $this.find('.group');
		var $childRows = $this.find('.row');
		var x = $childGroups.length;
		$childGroups.css({ 'height': 0 });
		$childRows.removeClass('active');
	}
	// =======================================================================================
	// EVENTS
	return {
		...
		execute: function () {
			...
			$menuL1Link = $menuDrawer.find('.l1').find('.row');
			$menuNoL1Link = $menuDrawer.find('.nol1').find('.row');
			$backBar = $menuDrawer.find('.back-bar');
			$menuBack = $menuDrawer.find('.icon-back');
			$groups = $menuDrawer.find('.group');
			$groupNoL1 = $menuDrawer.find('.group.nol1:not(.l2)');
			$groupNoL1Rows = $menuDrawer.find('.group.nol1').find('.row');
			$header = $('.header');
			...
			$menuNoL1Link.on('click', function () {
				track_click_row($(this));
				child = $(this).data('child');
				if (child) {
			...
			$(window).on('load', function () {
				...
				switch (variant) {
					case 'a1':
						...
						document.title = 'SLIDER+ACCORDION A1, Menu under, Icon no change, X overlay';
						break;
					case 'a2':
						$('body').addClass('menuA2');
						document.title = 'SLIDER+ACCORDION A2, Menu under, Icon turns red, X overlay';
						break;
					case 'a3':
						$('body').addClass('menuA3');
						document.title = 'SLIDER+ACCORDION A3, Menu under, Icon turns to X, No X overlay';
						break;
					case 'b1':
						$('body').addClass('menuB1');
						document.title = 'SLIDER+ACCORDION B1, Menu over, X overlay';
						break;
					case 'b2':
						$('body').addClass('menuB2');
						document.title = 'SLIDER+ACCORDION B2, Menu over, No X overlay';
						break;
				}
				//lg(`variant: ${variant}`);
